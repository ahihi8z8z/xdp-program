...
int load_minmax_from_csv(const char *filename, mlp_params *params) {
    FILE *fp = fopen(filename, "r");
    if (!fp) {
        perror("[ERROR] fopen minmax csv");
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%d,%lf,%lf", &feature_idx, &min_val, &max_val) != 3) {
            fprintf(stderr, "[WARN] Invalid line: %s\n", line);
            continue;
        }

        if (feature_idx < 0 || feature_idx >= MAX_FEATURES) {
            fprintf(stderr, "[WARN] Invalid feature index %d\n", feature_idx);
            continue;
        }

        // Convert float → fixed point (Q8.24)
        params->min_vals[feature_idx] = fixed_from_float(min_val);
        params->max_vals[feature_idx] = fixed_from_float(max_val);
        count++;
    }

    fclose(fp);
    printf("[INFO] Loaded %d min/max entries from %s\n", count, filename);
    return count;
}
...
int main(int argc, char **argv) {
    ...
    if (load_minmax_from_csv(minmax_path, &params) <= 0) {
        fprintf(stderr, "[WARN] Failed to load minmax params, using computed ones.\n");
    }
